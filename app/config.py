...
class CollaboratorModelSettings(BaseModel):
...
class ToolAgentModelSettings(BaseModel):
    provider: Literal["openai", "azure", "azure_openai", "zhipu", "gemini", "custom", "anthropic"] | None = None
    model: str | None = None
...
class ExternalToolSettings(BaseModel):
    ...
    market_snapshot_max_results: int = Field(
        ...
        ge=1,
        le=5,
        description="Upper bound for snapshot results returned per tool call",
    )
...
class BotSettings(BaseSettings):
    ...
    tool_agent: ToolAgentModelSettings | None = None
