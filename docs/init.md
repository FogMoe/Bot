...
- **Agent 架构**：
  ...
  - 短期上下文：所有用户、助手、工具消息都入库，`build_message_history` 会把最近记录传递给 Agent。
...
3. 发送消息 → 中间件扣减小时额度，消息入库。`AgentOrchestrator` 载入短期上下文和记忆摘要，调用 pydantic-ai，如果需要会触发工具。
4. 模型回复/工具输出都写入 `messages`，从而保证连续对话。
...
- **测试**：建议为 service 层（订阅、额度、记忆、搜索）和 Agent 历史构造器补充单元测试，确保后续 refactor 时行为稳定。
